...
class _CameraScreenState extends ConsumerState<CameraScreen>
  ...
  Future<void> _capturePhoto() async {
    ...
    try {
      ...
      final photo = await ref.read(cameraNotifierProvider.notifier).saveAndRegisterPhoto(image.path);
      if (photo != null) {
        ref.read(cameraNotifierProvider.notifier).onPhotoCaptured(photo.localPath);
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _importFromGallery() async {
    try {
      ...
      if (path != null) {
        final photo = await ref.read(cameraNotifierProvider.notifier).saveAndRegisterPhoto(path);
        if (photo != null) {
          ref.read(cameraNotifierProvider.notifier).onPhotoImported(photo.localPath);
        }
      }
    } catch (e) {
      if (mounted) {
