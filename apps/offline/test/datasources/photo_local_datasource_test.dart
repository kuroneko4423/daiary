...
void main() {
  ...
  group('getPhotos', () {
    test('returns all non-deleted photos', () async {
      ...
      await db.insert('photos',
          makePhoto('p3', deletedAt: DateTime.now().toIso8601String()));
    ...
    test('returns only trashed photos when trashOnly is true', () async {
      await db.insert('photos', makePhoto('p1'));
      await db.insert('photos',
          makePhoto('p2', deletedAt: DateTime.now().toIso8601String()));
  ...
  group('restorePhoto', () {
    test('clears deleted_at', () async {
      await db.insert('photos',
          makePhoto('p1', deletedAt: DateTime.now().toIso8601String()));
  ...
  group('searchByTags', () {
    test('finds photos by tag substring', () async {
      await db.insert(
          'photos', makePhoto('p1', aiTags: 'sunset,beach,ocean'));
      await db.insert(
          'photos', makePhoto('p2', aiTags: 'mountain,forest'));
